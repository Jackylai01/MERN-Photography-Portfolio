...
import CommercialImg from "../../components/CommercialImg/Photo";
...
const PatternOne = () => {
  ...
  async function handleAlldelete(e) {
    ...
    if (window.confirm("確定要全部刪除?")) {
      await axios.delete(`${FetchUrl}deleteCommercial`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  useEffect(() => {
    const fetchPhoto = async () => {
      const res = await axios.get(`${FetchUrl}getCommercial`);
      setData(res.data);
