...
import ProductImg from "../../components/productImg/Photo";
...
const Product = () => {
  ...
  //全部圖片刪除
  async function handleAlldelete(e) {
    e.preventDefault();
    if (window.confirm("確定要全部刪除?")) {
      await axios.delete(`${FetchUrl}deleteProduct`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  useEffect(() => {
    const fetchPhoto = async () => {
      const res = await axios.get(`${FetchUrl}getProduct`);
      setData(res.data);
      let newData = [];
        ...
        >
          {photo.map((product) => (
            <React.Fragment key={product}>
              <ProductImg product={product} productData={data} />
            </React.Fragment>
          ))}
        </PhotoProvider>
      </div>
